...
public class ResponseCode extends RemotingSysResponseCode {
    ...
    /**
     * 消息发送的返回状态之一：表示没有在规定时间内完成刷盘(需要Broker的刷盘策被设置成SYNC FLUSH才会报这个错误)
     */
    public static final int FLUSH_DISK_TIMEOUT = 10;

    /**
     * 消息发送的返回状态之二：这个状态产生的场景和FLUSH_SLAVE_TIMEOUT类似，表示在主备方式下，并且Broker被设置成SYNC_MASTER，但是没有找到被配置成Slave的Broker
     */
    public static final int SLAVE_NOT_AVAILABLE = 11;

    /**
     * 消息发送的返回状态之三：表示在主备方式下，并且Broker被设置成SYNC_MASTER方式，没有在设定时间内完成主从同步
     */

...
public class DefaultMQPushConsumerTest {
    ...
    public void testPullMessage_SuccessWithOrderlyService() throws Exception {
        ...
        /*
         * 这里设置一个回调函数：在MessageListenerOrderly的实现中，为每个Consumer Queue加个锁，消费每个消息前，需要先获得这个消息对应的
         * Consumer Queue所对应的锁，这样保证了同一时间，同一个Consumer Queue的消息不被并发消费，但不同Consumer Queue的消息可以并发处理
         */

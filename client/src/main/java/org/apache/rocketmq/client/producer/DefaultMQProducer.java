...
public class DefaultMQProducer extends ClientConfig implements MQProducer {
     ...
     * DefaultMQProducerImpl类用于具体实现DefaultMQProducer中的方法（这里采用组合的方式建立类之间的关系）
     */
    protected final transient DefaultMQProducerImpl defaultMQProducerImpl;

    /**
     * 创建producer时必须要指定一个group，这里有两个作用：
     * 1. 生产者一般会是集群部署的，group用来标识一类生产者，相同group的生产者一般要有相同的发送逻辑。
     * 2. 在发送事务消息时，当事务消息异常，broker端来回查事务状态时，需要知道是由哪类生产者发送的事务消息，
     *    生产端会根据group名称来查找对应的producer来执行相应的回查逻辑。
     *
     ...
     * broker 创建的默认的名为 AUTO_CREATE_TOPIC_KEY 主题Topic
     */
     ...
     * 消息重试时选择另外一个Broker是否不等待存储结果就返回，默认为false
     */
     ...
     * 通过构造函数创建DefaultMQProducer对象时，会同时把DefaultMQProducerImpl对象也创建出来
     *
     ...
     * 在broker节点上创建一个主题
     *
     * @param key accesskey 目前未做实际使用 可以与 newTopic 相同

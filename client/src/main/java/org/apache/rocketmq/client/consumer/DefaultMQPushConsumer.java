...
/**
 * 推模式消息消费者 实现类：
 * 消息队列负载机制：一个消费队列同一时间只允许被一个消费者消费
 *                一个消费者可以消费多个消费队列
...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
     ...
     * </li>
     * </ul>
     * 根据消息进度,当从消息服务器拉取不到消息时，重新计算消费策略
     */
     ...
     */
    private Map<String /* topic */, String /* sub expression：消息过滤表达式，TAG或 SQL92表达式。 */> subscription = new HashMap<String, String>();
    ...
    /**
     * 并发消息消费时，处理单个消息消费队列（consumerQueue）最大跨度，默认2000,
    ...
    /**
     * 默认值1000：流控制阈值在队列级别，每个消息队列默认最多缓存1000条消息，瞬时值可能超过极限
     *
    ...
    /**
     * 消息并发消费时一次消费消息条数，通俗点说 就是每次传入 MessageListtener#consumeMessage 中的消息条数
    ...
    /**
     *  最大消费重试次数。如果消息消费次数超过 maxReconsume­ Times还未成功，则将该消息转移到一个失败队列 ，等待被删除

...
/**
 * Brokder端处理消息拉取的类---执行请求唤醒，即再次拉取消息
 */
public class PullMessageProcessor implements NettyRequestProcessor {
    ...
    private RemotingCommand processRequest(final Channel channel, RemotingCommand request, boolean brokerAllowSuspend) throws RemotingCommandException {
        ...
        // 响应时会将客户端的请求序号（AtomicInteger自增值），设置进去进行返回
        ...
        // 计算系统标识：请求中的系统标识中涵盖了 该请求是否要挂起（有一个最大挂起时间），该请求是否要标记消费进度位置，标记是否过滤订阅表达式（配合过滤表达式使用）
        ...
        if (getMessageResult != null) {
            ...
            responseHeader.setNextBeginOffset(getMessageResult.getNextBeginOffset());// 待查找的队列偏移量
            responseHeader.setMinOffset(getMessageResult.getMinOffset());// 当前消息队列最小偏移量
            responseHeader.setMaxOffset(getMessageResult.getMaxOffset());// 当前消息队列最大偏移量
            ...
            switch (response.getCode()) {
                case ResponseCode.SUCCESS:
                    ...
                    // 读取消息
                    if (this.brokerController.getBrokerConfig().isTransferMsgByHeap()) {// 内存中读取
                ...
                case ResponseCode.PULL_NOT_FOUND:
                    ...
                    if (brokerAllowSuspend && hasSuspendFlag) {
                        ...
                        // 根据是否开启了长轮训，来设置拉取时间（短轮训时1s,长轮训时15s）
                        ...
                        // 调用 挂起 拉取消息的服务，将这个请求放到放到一个ConcurrentHashMap集合中

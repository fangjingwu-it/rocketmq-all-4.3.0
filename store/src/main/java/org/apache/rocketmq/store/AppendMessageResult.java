...
public class AppendMessageResult {
     ...
     *
     * 开始写的位置，消息真正的物理位置（偏移量），参考CommitLog的调用，等于fileFromOffset（当前文件的起始偏移量）+ byteBuffer.position()（该文件下一个要被读或写的元素的索引）;
     */
    ...
    /**
     * Consume queue's offset(step by one)消息的逻辑地址，代表是这个队列中的第几个位置，即下标
     * 消息消费队列逻辑偏移量，类似于数组下标 。
     */
    ...
    // 消息条数（只有在处理批量消息MessageExtBatch时，会设置为其他值）
